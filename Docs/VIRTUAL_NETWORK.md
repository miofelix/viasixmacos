...
- ad-hoc 构建没有可信 Team 身份，因此改用管理员安装器把完整 App 复制到 `/Library/Application Support/com.felix.viasix/InstalledApp/ViaSix.app`，由绝对路径 LaunchDaemon 启动 helper。root-only 策略同时绑定 App 与 helper 的精确 CDHash；任一文件变化都会 fail closed，并要求重新修复服务。禁止只按 bundle identifier 信任 ad-hoc 客户端，也禁止从用户可写的 `dist/` 直接启动 root helper。
...
- 隔离 Mac 或虚拟机上的真实 TUN、DNS 泄漏和卸载回归；ad-hoc 构建用于本机管理员安装与精确 CDHash 更新回归，但不把它当作可分发 TUN 发布包；
