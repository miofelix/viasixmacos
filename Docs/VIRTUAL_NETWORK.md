...
ViaSix 同时支持登录用户态本地代理/系统代理和特权 TUN。三种接入方式由 `NetworkAccessMode` 互斥协调；选择 TUN 前必须完成 LaunchDaemon 批准、固定签名运行时安装和 helper 能力检查。启动时 helper 等待新 `utun` 接口与回环 Controller 同时就绪，停止时撤销会话并等待接口消失。
...
- ad-hoc 构建没有可验证的 Developer ID Team 身份，真实 SMAppService/TUN 注册通常会显示“待批准”或“不可用”；只能把它用于用户态代理和 UI 状态验证。
...
服务注册、重新注册和特权 Mihomo 安装只允许在 journal 已确认 `inactive` 时执行。应用从“系统设置 → 登录项”回到前台后会重新读取 `SMAppService` 与 helper 状态；不会要求用户依赖旧的批准状态，也不会为了刷新而修改路由或 DNS。
...
- 隔离 Mac 或虚拟机上的真实 TUN、DNS 泄漏和卸载回归；在 ad-hoc 构建上只验证 UI 的未安装/待批准状态，不把它当作可用 TUN 发布包；
