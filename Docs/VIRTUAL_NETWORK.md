...
ViaSix 同时支持登录用户态本地代理、macOS 系统代理和特权 TUN。系统代理与 TUN 是独立开关，可以同时启用；选择 TUN 前必须完成 LaunchDaemon 授权、固定签名运行时安装和 helper 能力检查。启动时 helper 等待新 `utun` 接口与回环 Controller 同时就绪，停止时撤销会话并等待接口消失。
...
5. 如系统代理偏好已开启，其 HTTP、HTTPS 与 SOCKS 端点必须指向同一 Mihomo 的回环 mixed 端口，且不得影响 Mihomo 自身的上游绕行。
...
`NetworkAccessMode` 只选择用户态本地运行或特权 TUN 运行，`systemProxyEnabled` 独立控制 macOS 系统代理。切换 TUN 运行方式必须串行执行：
...
切换期间先恢复旧系统代理快照，目标监听就绪后按独立偏好重新应用系统代理。TUN 启动成功但系统代理应用失败时保留 TUN 会话并明确报告系统代理错误；应用退出必须等待当前切换收敛。
