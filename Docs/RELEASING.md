...
应用不安装系统扩展或网络扩展。应用包包含一个由 `SMAppService` 管理的 LaunchDaemon helper，以及与当前应用架构一致的固定 Mihomo v1.19.29。管理员必须在系统设置批准服务后 helper 才有资格运行。应用仍可用登录用户权限运行本机代理；只有完成特权运行时、路由、DNS 与恢复验证后才能开放虚拟网卡控制项。helper 不能执行用户目录中的二进制。
