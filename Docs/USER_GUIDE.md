...
ViaSix 提供本地端点，并有两个独立的系统接入开关：

- **本地代理**：只处理手动指向本地端点的应用。
- **系统代理**：把当前启用的 macOS 网络服务指向本地端点，只影响遵循系统代理的应用。
- **虚拟网卡**：由固定签名 Mihomo 通过特权后台服务创建 TUN，管理自动路由、DNS 劫持、IPv4/IPv6 和回环防护；可与系统代理同时启用。
...
2. 本地 ad-hoc 构建会弹出管理员授权，并安装 root-owned 固定 App 副本；正式签名版本如果显示“待批准”，请在系统设置“通用 → 登录项”允许 ViaSix 后台服务。
3. 回到 ViaSix 后，应用会自动刷新服务状态；也可以手动点击“刷新”。随后在“特权 Mihomo”行点击“安装”或“修复”。服务安装完成后，日常启动、停止和切换 TUN 都不会再次要求密码；只有特权 helper 或协议升级不兼容时才需要“修复服务”。
4. 看到服务与特权运行时均为“已就绪”后，才可在“设置 → 本机代理”选择“虚拟网卡”。

TUN 选项包括网络栈（Mixed/System/gVisor）、MTU（1280–9000）和严格路由。首次启动会生成受签名和摘要保护的特权配置；Controller 仍只绑定 `127.0.0.1` 并使用本机随机密钥。运行中切换进入或离开 TUN 时，ViaSix 会安全停止当前运行时、保存 TUN 偏好并自动启动目标运行时；系统代理偏好保持不变，并在新端点就绪后重新应用。

### 2.3 选择代理模式并配置服务器

1. 在“首页”或“设置 → 本机代理”中选择“规则”“全局”或“直连”。
2. 使用“规则”或“全局”时，打开“配置”。
3. 导入 `.yaml` 或 `.yml` 格式的 Mihomo 配置。受支持的单节点配置可通过“可视化编辑”查看和修改协议、端口、认证信息、传输及 TLS / REALITY 参数；多节点、Proxy Provider、代理组和规则使用 YAML 编辑器。
...
4. 按需独立启用“系统代理”和“TUN 模式”。TUN 需要先完成上一节的服务批准与特权 Mihomo 准备；运行中切换 TUN 时 ViaSix 会自动重启代理运行时。
5. 打开本地代理开关。
...
- 关闭系统代理、停止代理或退出 ViaSix 时，会先恢复启用前保存的 macOS 代理设置；只关闭 TUN 不会改变系统代理偏好。应用异常中断后，下次启动也会尝试完成恢复。
...
| `networkAccessMode` | `localProxy` 或 `virtualInterface`；旧 `systemProxy` 值会自动迁移 |
| `systemProxyEnabled` | 是否独立启用 macOS 系统代理，可与 `virtualInterface` 同时启用 |
...
- 打开“设置 → 虚拟网卡服务”，依次确认系统服务已完成授权、特权 Mihomo 已就绪且没有待恢复会话。
- 正式签名版本可从 macOS“系统设置 → 通用 → 登录项”允许服务；本地构建则点击“安装服务”或“修复服务”并批准管理员提示。回到 ViaSix 后必要时手动点击“刷新”。
- 如果显示“其他用户运行中”或“等待会话所有者处理”，请切换到启动该会话的登录用户完成停止或恢复；当前用户不会接管或重装正在使用的服务。
- ad-hoc 开发包通过 root-owned 固定副本、helper 精确 CDHash、固定用户和协议校验支持本机 TUN 调试。普通 App 重编可继续复用服务；只有 helper 或协议不兼容时需要重新修复。它仍不具备可信开发者身份，不可作为对外发布包；分发版本必须使用 Developer ID 签名、公证并完成隔离网络验证。
