...
3. 回到 ViaSix 后，应用会自动刷新服务状态；也可以手动点击“刷新”。随后在“特权 Mihomo”行点击“安装”或“修复”。本地 App 每次重新构建后都要再次“修复服务”，使精确 CDHash 与新构建一致。
...
- ad-hoc 开发包通过 root-owned 固定副本和 App/helper 精确 CDHash 支持本机 TUN 调试；每次重新构建都必须重新修复服务。它仍不具备可信开发者身份，不可作为对外发布包；分发版本必须使用 Developer ID 签名、公证并完成隔离网络验证。
