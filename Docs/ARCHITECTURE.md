...
- `ViaSixTunHelper`：最小权限 LaunchDaemon；当前仅做签名受限的能力探测与恢复协议验证，不启动代理内核，也不修改路由或 DNS。
...
- `Mihomo/controller.secret`：首次启动时生成的随机 Controller 鉴权密钥，权限为 `0600`，不进入特权配置投影。
...
| `networkAccessMode` | String | `localProxy` | `localProxy`、`systemProxy`、`virtualInterface`；当前拒绝启动后者 |
...
用户态 Mihomo 的 Controller 固定绑定 `127.0.0.1`，端口来自 `controllerPort`，每个请求使用本机随机密钥进行 Bearer 鉴权。导入 YAML 时会剥离 `external-controller` 和 `secret`，避免远端配置扩大监听范围或替换密钥；供特权/TUN 路径使用的配置投影也不包含 Controller 字段。
...
当前版本没有可用的虚拟网卡/TUN、默认路由接管或系统 DNS 重写能力。界面展示“虚拟网卡”作为接入方式，但控制项不可启用；配置与启动路径也会拒绝该模式。应用包中的 helper 只提供签名受限的协议骨架，不执行网络修改。完整安全边界见[虚拟网卡能力边界](VIRTUAL_NETWORK.md)。
...
| 特权 helper | 固定 identifier、相同 Team ID、UID/audit session 与固定 XPC methods | 非法客户端、协议混淆、特权接口过宽、恢复失败 |
| 虚拟网卡后端 | 当前明确不可用；未来需签名内核、路由/DNS 验证和精确恢复 | 默认路由回环、DNS 泄漏、外部路由修改 |
