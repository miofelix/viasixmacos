...
- `local-proxy.json`：本机监听、协议行为、代理模式、系统代理偏好和 TUN 运行方式。
...
| `networkAccessMode` | String | `localProxy` | 当前写入 `localProxy` 或 `virtualInterface`；旧 `systemProxy` 值作为迁移别名 |
| `systemProxyEnabled` | Bool | `false` | 独立的 macOS 系统代理偏好，可与 `virtualInterface` 同时为真 |
| `tunStack` | String | `mixed` | `mixed`、`system`、`gvisor` |
| `tunMTU` | Int | `1500` | 1280–9000 |
| `tunStrictRoute` | Bool | `false` | Mihomo 严格路由开关 |

解码旧文件时，`networkAccessMode: systemProxy` 迁移为 `networkAccessMode: localProxy` 与 `systemProxyEnabled: true`；旧日志级别 `none` 迁移为 `silent`。新文件独立保存系统代理和 TUN 偏好。
...
1. 仅在 Mihomo 本地 mixed 入站启动成功且 `systemProxyEnabled == true` 时继续。
...
保存 `systemProxyEnabled == true` 不代表 macOS 此刻已启用代理；只有 Mihomo 运行且系统配置事务成功时，UI 才报告系统代理已启用。

## 网络接入边界

路由模式决定“进入 Mihomo 之后如何出站”，网络接入方式决定“哪些应用会进入 Mihomo”。本地代理只接收显式配置端点的应用；系统代理只影响遵循 macOS 代理设置的应用。

TUN 是独立于系统代理的流量接入层。AppModel 只有在 LaunchDaemon 已批准、固定签名 Mihomo 已就绪且 helper feature set 完整时才允许选择和启动。helper 从 typed envelope 重新生成配置，以固定 `-d`/`-f` 参数启动 root-owned Mihomo，负责自动路由、严格路由、DNS hijack、IPv4/IPv6、回环防护、网络变化状态和 journal 恢复。会话按登录用户 UID 单一归属；非所有者只能观察脱敏状态，不能停止、恢复或通过重注册服务影响现有会话。应用重新激活时会刷新服务批准与 helper 状态。系统代理与 TUN 可同时应用；运行期间切换 TUN 会自动安全停止旧运行时、保存偏好并启动目标运行时。完整安全边界见[虚拟网卡能力边界](VIRTUAL_NETWORK.md)。
...
| 特权 helper | 正式包使用相同 Team ID；本地包以 root-owned 策略固定已安装 App/helper CDHash 和授权 UID；日常连接再校验 identifier、audit session、协议与固定 XPC methods | 非法客户端、协议混淆、特权接口过宽、恢复失败 |
...
正式分发面向 Developer ID 签名、公证和非 Mac App Store 安装。包含 LaunchDaemon 的 app 必须公证，并建议安装到 `/Applications`；helper 或 plist 更新后必须先异步 unregister，等待完成后再 register。本地 ad-hoc 构建走独立管理员安装路径：完整 App 固定复制到 root-only 系统目录，legacy LaunchDaemon 只从该副本启动 helper，策略固定已安装 App/helper CDHash 和授权用户。普通 App 重编通过兼容 XPC 继续复用服务，只有特权 helper 或协议不兼容才重新安装。普通本机代理仍可使用 Application Support 中由用户管理的 Mihomo，CFST 也以普通用户权限运行；helper 不能执行用户可写 Runtime 中的程序。供特权路径使用的固定 Mihomo 位于 app 的 `Contents/Library/HelperTools/`，并由签名后的 `PrivilegedRuntime.plist` 绑定完整摘要和 CDHash。
