...
4. 在“设置”中点击“配置服务器”，填写协议、服务器地址、认证信息、端口、传输和 TLS / REALITY 参数。
5. 如需修改监听地址、端口或 UDP，打开“本机代理设置”。
6. 回到“连接”打开本地代理开关。
7. 在需要使用代理的应用中填写“设置”里显示的本地端点。

ViaSix 使用服务器配置和本机代理设置生成 Xray 运行配置。同一个回环端口可作为 HTTP 或 SOCKS5 代理使用。ViaSix 不会自动修改 macOS 系统代理。服务器表单支持 VLESS、VMess、Trojan 和 Shadowsocks，也可以粘贴对应的分享链接；特殊 Xray 出站可以使用高级 JSON 编辑器。
...
应用内置的是不含真实账号的安全示例配置。请在“设置”中点击“配置服务器”填写自己的连接参数，或使用高级 JSON / 完整配置导入功能。
