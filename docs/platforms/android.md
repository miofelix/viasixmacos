...
- 全量隧道：`Tun2SocksEngine` — `Builder.setBlocking(true)` 提供与 `FileChannel.read` 一致的阻塞 TUN fd；SYN-ACK 使用稳定服务端 ISN，重复 SYN 可重发，只有有效客户端 ACK 才释放远端下行，10 秒超时自动清理；TCP 序列号按无符号 32 位回绕处理，重复前缀去重、未来乱序段拒绝，客户端 FIN 使用 socket 半关闭且远端 EOF 回送 FIN；解析客户端 16 位 advertised window，ACK 只在已原子登记的发送边界内推进，拒绝确认未发送数据和倒退确认，零窗口或未确认数据占满窗口时暂停远端读取，并按剩余窗口限制单次读取大小；下行未确认段按会话有界保留，由共享扫描线程按 1–8 秒指数退避重传，累计/部分 ACK 可释放或裁剪缓存，短连接在数据确认后再发送 FIN，重试耗尽则关闭会话；TUN 出站队列区分不可静默丢弃的 TCP SYN/data/FIN/RST 与可丢数据报，TCP 队列饱和会向远端 socket 读取施加有界背压，UDP 不能淘汰 TCP 段；IPv4/IPv6 TCP→SOCKS5 CONNECT；通用 UDP→**每本地源端口** SOCKS5 UDP ASSOCIATE（正确并发 demux）；DNS 的 TCP/UDP 默认经 SOCKS，也可显式切换为 `protect` 直连并自定义数字 IPv4/IPv6 服务器，TCP 套接字在连接前完成保护，UDP 直连查询在创建 worker/socket 前受 32 个 in-flight permit 的硬上限约束
