...
- TUN 帧边界：IPv4 total length、IPv6 payload length、TCP data offset/l4 length 与 UDP length 必须全部落在实际读取帧内；畸形帧只记录并丢弃，读线程的 `running=false` 放在 `finally` 中，避免异常后监督器误报存活
