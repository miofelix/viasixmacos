...
- 全量隧道：`Tun2SocksEngine` — `Builder.setBlocking(true)` 提供与 `FileChannel.read` 一致的阻塞 TUN fd；TCP 序列号按无符号 32 位回绕处理，重复前缀去重、未来乱序段拒绝，客户端 FIN 使用 socket 半关闭且远端 EOF 回送 FIN；IPv4/IPv6 TCP→SOCKS5 CONNECT；通用 UDP→**每本地源端口** SOCKS5 UDP ASSOCIATE（正确并发 demux）；DNS 的 TCP/UDP 默认经 SOCKS，也可显式切换为 `protect` 直连并自定义数字 IPv4/IPv6 服务器，TCP 套接字在连接前完成保护
