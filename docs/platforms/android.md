...
- 全量隧道：`Tun2SocksEngine` — `Builder.setBlocking(true)` 提供与 `FileChannel.read` 一致的阻塞 TUN fd；IPv4/IPv6 TCP→SOCKS5 CONNECT；通用 UDP→**每本地源端口** SOCKS5 UDP ASSOCIATE（正确并发 demux）；DNS 的 TCP/UDP 默认经 SOCKS，也可显式切换为 `protect` 直连并自定义数字 IPv4/IPv6 服务器，TCP 套接字在连接前完成保护
