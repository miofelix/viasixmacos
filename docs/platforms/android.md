...
- TUN 帧边界：IPv4 total length、IPv6 payload length、TCP data offset/l4 length 与 UDP length 必须全部落在实际读取帧内；IPv6 在声明 payload 内最多遍历 8 层 Hop-by-Hop、Routing、Destination Options、AH 与 Fragment 扩展头，仅接受无需重组的原子 Fragment，并把扣除扩展头后的真实偏移/长度交给 TCP/UDP 解析；畸形帧只记录并丢弃，读线程的 `running=false` 放在 `finally` 中，避免异常后监督器误报存活
